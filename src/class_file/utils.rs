...
pub fn parse_generic_signature<'a>(
    class_file_name: &str,
    pool: &'a ConstantPool,
    length: u32,
    buf: &mut Bytes,
    ...
    assert!(generic_signature.is_none(), "Duplicate generic signature attribute found for {} in \
        class file {}!", type_name, class_file_name);
