...
pub fn parse_generic_signature(
    ...
    assert!(length == 2 || buf.len() < 2, "Invalid generic signature attribute for {} in class_file \
        file {}! Expected length of 2, was {}!", type_name, class_file_name, length);
    let index = buf.get_u16();
    assert!(pool.has(index as usize), "Invalid generic signature attribute for {} in class_file \
        file {}! Expected index {} to be in constant pool!", type_name, class_file_name, index);
    let tag = pool.get_tag(index as usize)
        .expect(&format!("Invalid generic signature attribute for {} in class_file file {}! Expected \
            value at index {} in constant pool!", type_name, class_file_name, index));
    assert_eq!(tag, &UTF8_TAG, "Invalid generic signature attribute for {} in class_file file {}! Expected UTF-8 \
        string at {}, was {}!", type_name, class_file_name, index, tag);
    let value = pool.get_string(index as usize);
    assert!(value.is_some(), "Invalid {} in class_file file {}! Expected generic signature to be at \
